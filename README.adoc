...
To integrate https://stormpath.com[Stormpath] for user authentication, you'll first need to https://api.stormpath.com/register[register] and download an API key.

.Stormpath Setup
****
The https://docs.stormpath.com/java/spring-boot-web/quickstart.html[Stormpath Spring Boot Quickstart] shows how to create a Stormpath account and download an API key; here’s the abridged version:

. Signup for a free Stormpath developer account at https://api.stormpath.com/register.
. From the Home tab of the Admin Console select Manage API Keys under the Developer Tools heading.
. Click the Create API Key button to trigger the download of an apiKey-{API_KEY}.properties file.
. Move the file to `~/.stormpath/apiKey.properties`.
****

Install the https://github.com/stormpath/stormpath-sdk-angular[Angular components for Stormpath] using npm.

----
yarn add angular-stormpath
----

NOTE: You can also use `npm install --save angular-stormpath`, but https://code.facebook.com/posts/1840075619545360[Yarn] is faster.

Modify `app.module.ts` to import `StormpathModule` and configure your app to use https://docs.stormpath.com/client-api/product-guide/latest/[Stormpath's Client API].
...
<div *ngIf="(user$ | async)" class="row text-center">
  ...
  <ul class="nav nav-pills nav-stacked text-centered">
    <li role="presentation" (click)="logout(); false"><a href="#" id="logout">Logout</a></li>
  </ul>
</div>
<!-- Routed views go here -->
...
After making these changes, you should be able to run `ng serve` and login (or register) with one of your user's that's configured in Stormpath.

[[stormpath-login]]
.Stormpath login
image::src/assets/images/stormpath-login.png[Stormpath login, 800, scaledwidth="100%"]

After logging in, you'll be able to search and see the user's information.

[[stormpath-post-login]]
.View after login
image::src/assets/images/stormpath-post-login.png[View after login, 800, scaledwidth="100%"]

If it works - congrats! If it doesn't, let us know in our https://talkstormpath.slack.com[Talk Stormpath] Slack channel, or hit me up https://twitter.com/mraible[on Twitter].
